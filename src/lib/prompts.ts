...
export function campaignFitPrompt(role: string, company: string, jd: string, background?: string, storyContext?: string): string {
  ...
  const backgroundBlock = hasBackground
    ? `Candidate's background/resume:\n${background!.trim()}\n\nUse this background to make the fit analysis genuinely personal to this candidate: cite specific experience from it in "strengths", and name real gaps between what they've actually done and what this JD needs — not generic PM gaps. Personal and specific does not mean longer — stay within the word counts given below even when there's more you could cite.`
